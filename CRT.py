...
def poly_CRT(equations):
	Mt = reduce(lambda acc, val: poly_mul(acc, val), map(lambda x: x[1], equations), (1, ))
	M = tuple(map(lambda x: poly_division(Mt, x[1])[0], equations))
	l = [None] * len(equations)

	sol = (0, )
	for i in range(len(equations)):
	
		l[i] = poly_euclid_core(M[i], equations[i][1])[1 if len(M[i]) > len(equations[i][1]) else 2]
		curr_product = poly_mul(equations[i][0], poly_mul(l[i], M[i]))
		sol = poly_sum(sol, curr_product)

	return (poly_division(sol, Mt)[1], Mt)
